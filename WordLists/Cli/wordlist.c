// System dictionaries  generated by parsewords.py  Nov 09, 2016  11:49:19
...
// Kernel
	...
	// SFP
	NAME("initsfp")		//  initialize sfp firmware
	NAME("sfpstats")		//  show sfp statistics
	NAME("nodestats")		//  node stats
	NAME("initsfpstats")		//  zero out all sfp stats
	NAME("reboot")		//  reboot the software
	NAME("checksum")		//  ( a n - c ) return checksum from a for n bytes
//	su sendData // ( a n - ) send n bytes at address a over usb
	NAME("lf")		//  dump out pool of frames 
	// copy and run
	NAME("car")		//  ( a - )copy and run an application from flash
	NAME("copyapp")		//  ( a - f ) copy application
	NAME("runapp")		//  ( a - f ) run application
	NAME("vh")		//  ( a -- ) check header checksum
	NAME("vi")		//  ( a -- ) check image checksum
	NAME("dh")		//  ( a -- ) dump header parameters of an image
	// rtc
	NAME("date")		//  show date and time
	NAME("setdate")		//  ( Y M D - ) set the date
	NAME("settime")		//  ( H M S - ) set the time
	// stm bootloader call
	NAME("stmboot")		//  invoke system bootloader so new firmware can be downloaded
END_NAMES
...
NAMES(constantnames)
	...
	NAME("iflash")		//  start of internal flash
END_NAMES
